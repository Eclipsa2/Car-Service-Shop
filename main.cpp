...
Vehicul* Vehicul::Creare(int tip) {
    if(tip == 1)
        return new Masina;
    else
...
static Vehicul vehiculPolitie("MAI", "alba", false, 2022, 120);

Singleton *Singleton::instance = 0;
int main() {
    Singleton *s = s->getInstance();
    Masina* masinaPolitie = new Masina("dacia", "duster", &vehiculPolitie);
    ...
    s->citireFisierReparate(nrMasiniReparate, nrMotocicleteReparate, vehiculeReparate);
    s->citireFisierService(nrMasiniReparate, nrMotocicleteService, vehiculeService);
    int optiune = menu();
    while (optiune != 0)
    {
        switch (optiune)
        {
            case 1:
            {
                ...
                vehiculeService.push_back(aux->Creare(1));
            ...
            case 6:
            {
                ...
                vehiculeService.push_back(aux->Creare(2));
                auto p = vehiculeService.end() - 1;
    ...
    s->scriereFisierReparate(nrMasiniReparate, nrMotocicleteReparate, vehiculeReparate);
    s->scriereFisierService(nrMasiniService, nrMotocicleteService, vehiculeService);
    cout<<endl<<"O zi buna!\n";
